...
- 推荐职责分工：小树脂（约 +15）用于常规暗路补给；暖石/萤火草（约 +25）作为重要喘息点；公共篝火提供停留、续火和留下温暖的选择，而非无代价补满余火。
...
- `MarshmallowRitual`：与 RestSpot 同物体，标记可烤；在篝火火焰中心上方创建空物体 `MarshmallowLookTarget`（建议约高于地面 1.0–1.4m），并赋给仪式的 `Look Target`。停留时玩家会水平转向篝火、锁定常规视角并平滑看向该目标；第一次按 Q 消耗 `Materialize Fuel Cost`（默认 3）拟造棉花糖，此后 Q 翻面。离座会结束本次棉花糖，且不保存；
